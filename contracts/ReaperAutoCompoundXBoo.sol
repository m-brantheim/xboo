...
// bunch of imports you don't need here like Ownable, Pausable
// basically anything that was taken over by BaseStrategy
// also might want to import from OZ's npm package to avoid
// possibility of any copy-pasta error
...
// remember to remove this to further save some size
...
contract ReaperAutoCompoundXBoo is ReaperBaseStrategy {
     ...
     */
    address public wftm = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83); // can make constant
    address public xBoo; // can make immutable
    address public boo; // can make immutable
    // for xboo and boo you can actually declare them as IERC20/IBooMirrorWorld types even in the constructor
    // ABI will do the implicit conversion from address to IERC20.. might save you a few casts
    // through the contract. We do this in the stablecoin strat if i remember correctly.
     ...
     */
    // why not just hardcode these and make them constants?
    // at the very least make them immutable
     ...
     */
    address[] public wftmToBooRoute;// (you can just declare this inline) = [wftm, boo];
     ...
     */
    // FYI solidity will convert all smaller-byte ints into uint256 unless they're part of a struct where
    // packing comes into play. however for any other use case, it actually costs more gas if I'm not mistaken
    // see https://ethereum.stackexchange.com/questions/3067/why-does-uint8-cost-more-gas-than-uint256
     ...
     */
    // remember to update "strategist" to "strategistRemitter" when you integrate PaymentRouter
    ...
    constructor(
        address _uniRouter,
        address _aceLab,
        address _rewardToken,   // yup this and xBoo can be of type IERC20 then
        address _xBoo,
        address _vault,
        address _treasury,
        address _strategist
    ...
    function deposit() public whenNotPaused {
        ...
        if (booBalance > 0) { // for uints != 0 is the same as > 0 and costs less gas :)
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (booBalance < _amount) {
            for (
                ...
                // so we just pull from the first available pool?
                //
                // no need to pull from the pool that would have the least effect
                // on rewards?
                //
                // hmm i guess that should be okay since in the next harvest we'd
                // re-shuffle the funds anyway
                if (currentPoolxBooBalance > 0) {
                    ...
                    // in the spirit of the CEI pattern (checks -> effects -> interactions)
                    // you might want to update the pool balances first (effects) and then
    ...
    function removeUsedPool(uint8 _poolIndex) external {
        ...
        uint256 balance = poolxBooBalance[poolId];
        IAceLab(aceLab).withdraw(poolId, balance);
        // so at this point we potentially just have a bunch of xBoo tokens just sitting
        // in the strat? should we redposit them into currentPoolId or is that not a concern?
        totalPoolBalance = totalPoolBalance.sub(balance);
        poolxBooBalance[poolId] = 0;
        // honestly wondering if OpenZeppelin's EnumerableSet would
        // be perfectly suited for currentlyUsedPools
        // O(1) insert, deletion and contains() checks
        // O(n) enumeration
        // so it'll still be like traversing an array but you won't
        // have to do all this manipulation to avoid gaps in an actual array
