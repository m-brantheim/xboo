...
contract ReaperAutoCompoundXBoo is ReaperBaseStrategy {
     ...
     */
     // Can probably make boo and xboo constants and hardcoded, or use more generic naming to make the strategy easier to port
    ...
    // For greater precision and readability, you could use maxPoolDilutionFactor in a similar way to fees 
    // (e.g. use maxPoolDilutionFactor/PERCENT_DIVISOR with maxPoolDilutionFactor = 2000 for 20%)
        ...
        if (booBalance < _amount) {
            for (
                ...
                if (currentPoolxBooBalance > 0) {
                    ...
                    if (booBalance >= _amount) { // Can move this condition inside "for" condition
                        break;
                    }
                }
            }
    ...
    function _collectRewardsAndEstimateYield() internal {
        ...
        for (uint256 index = 0; index < nrOfUsedPools; index++) {
            ...
            poolxBooBalance[poolId] = 0; // Really nitpicky, but earlier you did : poolxBooBalance[poolId] = poolxBooBalance[poolId].sub(withdrawAmount)
            // May want to use the same pattern, or even pack this in an internal func
    ...
    function _swapRewardToWftm(uint8 _poolId) internal {
        ...
        if (poolRewardTokenBal > 0 && address(rewardToken) != wftm) {
            ...
            if (rewardToWftmPaths.length < 2) { // Perform this check when the pool is added is enough, isn't it ? 
     ...
     * @dev Swaps any pool reward token to wftm
     */
     //Documentation off + this function could use some commenting to make it easier to read/understand
    function _setEstimatedYield(uint8 _poolId) internal {
        ...
        multiplier = _to - _from; // Multiplier is set in if checks, then overwriten here. Should this line be moved before the if checks ?
        ...
        external
    {
        ...
        if (_poolRewardToWftmPath.length > 0) { // Previous require() guarantees _poolRewardToWftmPath.length >= 2
