...
contract BooMirrorWorld is ERC20("Boo MirrorWorld", "xBOO") {
        ...
        returns (uint256 booAmount_)
    {
        ...
        booAmount_ = xBOOAmount.mul(boo.balanceOf(address(this))).div(
            totalxBOO
        );
    }
    ...
    {
        uint256 totalxBOO = totalSupply();
        booAmount_ = _xBOOAmount.mul(boo.balanceOf(address(this))).div(
            totalxBOO
        );
    }
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "BOO::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "BOO::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "BOO::delegateBySig: signature expired"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "BOO::_writeCheckpoint: block number exceeds 32 bits"
        );
